...
#define BEND_SPEED_INNER 75
#define BEND_SPEED_OUTER 400
#define BASE_SPEED 30
#define SPEED_GAIN 8

#define CMD_MSG_TIMEOUT_DURATION 10
...
#define TURN_LEFT_DISTANCE 19
#define MAX_WALL_DISTANCE 13
#define IDEAL_WALL_DISTANCE 12
...
void loop() {
  ...
  float dist_front = ((sonic_1 + sonic_2 + sonic_7 ) / 3);
  //distance from ideal wall distance in decimal.
  float dist_from_ideal = abs(sonic_5 - IDEAL_WALL_DISTANCE);
  float from_ideal_factor = (1 - ((dist_from_ideal / IDEAL_WALL_DISTANCE)*0.9));

  float diff_left = ((abs(sonic_5 - sonic_6)/(dist_left*2)));
  //Angle related to left wall in degrees
  diff_left = constrain(diff_left, 0.5, 1);
  from_ideal_factor = constrain(from_ideal_factor, 0.5, 1);
  float angle_left = (1 - diff_left);
  Serial.println("Diff_left");
  Serial.println(diff_left);
  ...
  int correction_speed = (forwards_speed * from_ideal_factor * angle_left * CORRECTION_FACTOR);
  Serial.println("forwards_speed");
  Serial.println(forwards_speed);
  Serial.println("correction_speed");
  Serial.println(correction_speed);

  if (sonic_1 < STOP_DISTANCE or sonic_2 < STOP_DISTANCE or sonic_7 < STOP_DISTANCE) {
    stop_robot();
    stop_robot();
    turn_right();
...
/*
 ...
 Serial.println("Sonic nr. 7 distance is:");
  Serial.println(sonic_7);
  */
  delay(0);
